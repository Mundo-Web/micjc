    ...
    <form action="{{ route('prices.update') }}" method="POST">
        ...
        <div class="col-span-full xl:col-span-8 bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
            <header class="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
                <h2 class="font-semibold text-slate-800 dark:text-slate-100 text-2xl tracking-tight">Modificar costos de envio</h2>
            </header>
                          ...
                          <div class="relative mb-2  mt-2">
                            <select name="departamento_id" id="departamento_id"
                              ...
                              @foreach ($departamentos as $item)
                                <option value="{{ $item->id }}" @if($item->id == $prices->department)>{{ $item->description }}</option>
                              @endforeach
